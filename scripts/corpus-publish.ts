...
if (!existsSync(INVENTORY_FILE)) {
  process.stdout.write(
    `the curriculum is what says a card exists, and ${INVENTORY_FILE} is missing\n`,
  );
  process.exit(1);
}
...
if (
  ...
  process.stdout.write(
    "the tree carries changes, so no commit describes what would be published\n",
  );
  process.exit(1);
}
...
for (const needed of [
  ...
  if (!existsSync(needed))
    throw new Error(`${needed} is missing. Run pnpm corpus to write it`);
}
...
const words = Object.fromEntries(
  Object.entries(
    readMeanings(readFileSync(at("vocabulary.json"), "utf8")),
  ).flatMap(([word, glosses]) => {
...
const decompositions = readDecompositions(
  readFileSync("corpus/decomposition.json", "utf8"),
);

const { read } = walkCurriculum(
  subjects,
  names,
  (character) => decompositions.get(character) ?? [],
);
...
const short = subjects.flatMap((subject) =>
  ...
  wordFor(subject, wrote) === undefined
    ? [subject.characters]
    : [],
);
...
const database = await connect({
  ...
  directory:
    asOptional(process.env["TANUKITSUNE_LOCAL_DATABASE"]) ?? LOCAL_DATA_DIR,
});
...
process.stdout.write(
  `published: ${rows.length} of the ${subjects.length} subjects the curriculum deals, at ${version.slice(0, 8)}\n`,
);
process.stdout.write(owing);
