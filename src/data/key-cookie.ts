...
export function accountCookie(held: Held): string {
  return `${READER_ACCOUNT_COOKIE}=${encodeURIComponent(JSON.stringify(held))}; Path=/; Max-Age=${HELD_FOR}; HttpOnly; SameSite=Strict; Secure`
}
...
export type Held = { readonly username: string; readonly level: number }
