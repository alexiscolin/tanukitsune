...
// they hand back.
//
// The token and the host both travel as arguments rather than being read here or captured once,
// so this file is testable against a fake server, nothing in data/ decides what a deployment
...
async function batched<Entry>(
  ...
  for (let from = 0; from < ids.length; from += IDS_PER_REQUEST)
    batches.push(collect(client, collection, path(ids.slice(from, from + IDS_PER_REQUEST))))

  return (await Promise.all(batches)).flat()
}

function subjectsIn(client: Client, ids: readonly number[]): Promise<SubjectEntry[]> {
  return batched(client, subjectCollection, (batch) => `/subjects?ids=${batch.join(',')}`, ids)
}
...
async function written(client: Client, ids: readonly number[]): Promise<Map<number, StudyMaterial>> {
...
async function mentions(
  ...
  for (const entry of await subjectsIn(client, wanted))
...
async function listSubjects(client: Client, ids: readonly number[]): Promise<readonly Subject[]> {
  ...
  const named = await mentions(client, entries, granted)

  return entries.map((entry) => toSubject(entry, named, mine.get(entry.id)))
}

async function listWaiting(client: Client): Promise<Waiting> {
  const [lessons, reviews] = await Promise.all([
    collect(client, assignmentCollection, '/assignments?immediately_available_for_lessons'),
    collect(client, assignmentCollection, '/assignments?immediately_available_for_review'),
  ])
...
export function wanikaniSource(token: string, api: string = API): KnowledgeSource {
