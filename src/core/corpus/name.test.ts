...
const fault = (name: string, character = '口') => faultInName(name, character, FRENCH)

describe('faultInName', () => {
  it('accepts a name shaped like the ones already written', () => {
    expect(fault('la bouche')).toBeNull()
    expect(fault("l'arbre", '木')).toBeNull()
    expect(fault("le lit d'hôpital", '疒')).toBeNull()
  })
  ...
  it('refuses a name too long to sit inside a sentence', () => {
    expect(fault('le bec du petit canard', '嘴')).toBe('too long')
  })
  ...
  it('refuses the apostrophe the locale does not use', () => {
    expect(fault('le lit d’hôpital', '疒')).toBe('not the locale')
  })

  it('refuses a name that is the character it is meant to replace', () => {
    expect(fault('la 口 ouverte')).toBe('not the locale')
  })
