...
describe('carriesTheReading', () => {
  ...
  it('holds where the word says the reading and keeps going', () => {
    expect(carriesTheReading(phonemesOf('かわ'), ['k', 'a', 'w', 'a'], 0.15)).toBe(true)
    expect(carriesTheReading(['n', 'i', 'n'], ['n', 'i', 'n', 'a', 'ʒ'], 0.15)).toBe(true)
  })

  it('fails where the word drops a sound the reading makes', () => {
    expect(carriesTheReading(['i', 'tɕ', 'i'], ['i', 's', 'y'], 0.15)).toBe(false)
  })
  ...
  it('fails where the word runs out before the reading does', () => {
    expect(carriesTheReading(['k', 'a', 'w', 'a'], ['k', 'a'], 0.15)).toBe(false)
  })
  ...
  it('allows a sound the language hears as the one next to it', () => {
    expect(carriesTheReading(['k', 'o'], ['k', 'ɔ'], 0.15)).toBe(true)
    expect(carriesTheReading(['k', 'o'], ['k', 'a'], 0.15)).toBe(false)
  })
})
