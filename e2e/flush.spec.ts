...
async function taken(request: { get: (url: string) => Promise<{ json: () => Promise<unknown> }> }) {
  const body = (await (await request.get(`${sourceURL}/taken`)).json()) as { taken: unknown[] }
...
test('a session answered reaches the source once, and a replay sends nothing more', async ({
  ...
  const subjects = new Set(questions.map((question) => question.subject.id)).size

  await expect
    .poll(async () => (await taken(request)).length, { timeout: 15_000 })
    .toBe(before + subjects)
  ...
  expect(await taken(request)).toHaveLength(before + subjects)
})
