...
- **Nothing is gated at commit time**, because linting every commit blocks an agent mid-loop for no
  gain. CI checks the pull request title and every commit subject on the branch somebody wrote, both
  through one script so the two cannot drift. The style is the same on both so that the history stays uniform
