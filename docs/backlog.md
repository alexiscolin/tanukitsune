...
| Subjects and assignments cached in the browser | none | The spec's data model puts them there and nothing does yet: both routes read them from the source on the server, per render, so a session cannot start without a network. They are caches of server truth, replaced wholesale and never merged, so eviction costs a download rather than data. This and the service worker are the two halves of the offline promise, and the outbox is the third piece, which already exists. |
| The service worker | none | What makes the app open and run a session with no network at all, plus the deployment-skew handling `../framing.md` works out under rendering and caching: chunks published before the manifest, never the reverse. It arrives with the web app manifest that makes the app installable, which also exempts it from Safari's seven-day eviction. |
