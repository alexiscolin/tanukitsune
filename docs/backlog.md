...
| A session dealt by how long an item has been due | none | A sitting takes the front of the queue as the source lists it, which is paged by identifier rather than ordered by urgency. Dealing the most overdue first needs `available_at`, which the assignment parser does not read at all today. |
| A windowed deck strip | none | The strip draws the current subject and its neighbours and is handed the whole queue, which the ten-item sitting keeps small enough to ignore. It stops being ignorable the day the reader chooses a longer session, since the payload follows the queue rather than the five cards drawn from it. |
| A queue held to the levels the subscription grants | none | The ceiling is held where the subjects are read, because an assignment names a subject and not a level, so a queue can count more than a session deals. It costs a subscriber whose subscription lapsed after reaching a high level: their queue counts items at levels they no longer hold, and since a sitting takes the same front ten every time, those ten are filtered out and the session ends before it starts. Holding the queue to the ceiling needs a subject-to-level map, which is the browser cache above. |
| A deployment configured for an account refuses to deal the seeded deck | none | Which deck is dealt is read from the token alone, so a deployment that loses it serves the demo without saying so. Two costs, and both are permanent: the demo's answers are backed up like any others and carry real subject identifiers, so they enter an append-only table indistinguishable from real reviews; and a restart of that deck empties a queue that may still hold real answers nothing has taken. Gating the backup on the deck instead would cost the end-to-end suite every test of the drain, the suite's server holding no token, so the answer is the deployment failing loudly rather than a second condition at the wiring. It arrives with the fake source below, which is what lets a session be tested against something other than the seeded deck. |
| A queue the reader can see and a failure they are told about | none | It also shows the rows a walk took and never resolved, which carry a sync time and no verdict and which nothing puts back. A send that fails keeps its rows and says nothing, and the drain restarts from the oldest, so one batch a server persistently refuses holds everything behind it while the queue grows unseen. What the reader needs is the count and the last refusal, which is a surface rather than a condition, and it belongs with the session screen rather than inside the drain. |
| Pacing the flush from the rate-limit headers | none | The flush sends one submission per subject, serially, and reads nothing back about the budget it is spending. A sitting is ten items, so a flush is at most ten requests against sixty a minute and the reader never meets the ceiling; a backlog flushed after a break is what does. What it needs is the route returning the remaining and reset values the source sends, which `../framing.md` already asks for under mutation transport, and the loop in the page waiting on them. |
| A waiting service worker handed over when a session ends | none | It is handed over when the tab goes away, which covers a reader who leaves. One who finishes a session and stays on the start screen with the tab visible keeps the old worker until they do. `../framing.md` names both moments; the second needs the loop to say when it ended, which is a message from a screen that has no other reason to talk to a worker. |
| A cache over the source's reads | none | Both routes ask the account again on every navigation, and a session render reads the subscription twice, once where the queue is held to it and once where the subjects are. A count also walks every page of a queue to take its length, and a sitting walks every page to take ten, where the collection's own total and its first page would answer both. What is cacheable and for how long is the same question the flush answers, so it is decided with it rather than before. |
...
| Reveal and grade, without typing | none | Space reveals the answer, one key says it was known and another says it was not. An accessibility mode before a preference, since a reader whose hands tire stops reviewing rather than reviewing differently. The row it writes is the shape self-grade already has, a cascade that decided nothing and a reader who did, with `assist` naming the revealed reference. What it cannot produce is a labelled disagreement, since no tier was asked, so those rows stay out of the calibration set: the price of the mode rather than a defect in it. |
...
| Anyone reviews their own account | none | Specified in [`specs/v0.2.md`](specs/v0.2.md). The key is the identity, so what looked like the cost of multi-user is smaller than it reads: no password, no encryption at rest and no recovery, since the key travels in a cookie and is stored nowhere, per [ADR 0017](decisions/0017-a-key-lives-in-a-cookie.md). What stays is a deletion path and per-reader scoping on every read, the subscription check included, which v0.1 already makes against one account. |
...
| The stage of the item under review, shown | none | Which stage an item sits at says how much recall effort is worth spending on it: missing a low one costs a day, missing a high one costs weeks. It is read from the cached assignment, which is on the device before the question is asked; `review_event.srs_stage_before` records it afterwards and is written too late to render anything. Colour alone fails WCAG 1.4.1, so it carries a second channel. |
...
| Learning order control | none | The Reorder userscript exists because WaniKani refuses this. FSRS decides the order by default, from difficulty it models per card; this is the reader overriding that, by stage or by type, when they know something the model does not. Ascending stage puts the least consolidated items first, which is what someone with ten minutes needs and what a random draw takes from them. |
