...
| A queue held to the levels the subscription grants | none | A subscription granting nothing leaves no queue at all, but a ceiling between one and sixty is held where the subjects are read: an assignment names a subject and not a level, so what is counted as due can still name more than a session deals. Filtering it needs the subject, which is the request the count exists to avoid. |
