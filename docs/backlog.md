...
| Multi-user | none | Auth, token encryption, a deletion path, and per-user scoping on every read, the subscription check included, which v0.1 already makes against one account. Cut from v0.1 because each of those four touches code the single-user version does not have to write at all. |
