...
| A fake WaniKani for the end-to-end suite | none | Every session the suite runs is the seeded deck, because a server started with a token would deal whoever's account the machine holds and a suite asserting what is waiting would then pass or fail on somebody's study day. So nothing upstream of the deck is driven end to end, and the flush cannot be either. MSW over the source's own HTTP, which is what `AGENTS.md` allows it for, against a server the suite starts with a token nobody owns. |
