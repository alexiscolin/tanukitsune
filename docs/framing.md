...
**What is reachable without a token is ours; what the token reaches stays on the reader's device.** The
chunks and the demo are public by construction, so nothing generated by anyone else may travel in them.
What a token fetches is a copy held for the account that already has it, on its own machine, and a
...
The caching surface is small enough that this is not a sacrifice: the application shell, the static
...
browser already carries the answer a first screen would interrupt to ask. An explicit choice is offered
in the interface and wins over the header from then on. Two consequences travel with that: the
negotiated redirect varies on the header or a shared cache serves one language to everyone, and the
manifest entry point pins a language at install time, so the choice is resolved and stored before an
installed app exists rather than after.
